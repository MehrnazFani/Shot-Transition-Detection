...
. Input video:
    . '.mp4' videos of arbitrary frame rate and frame size
. Outputs:
    . video-shots,'.mp4': To speed up the process, user can select to reduce the frame size and frame rate (if input fps>=60) of the output video shots 
...
for file in os.listdir(videos_dir):
    ...
    if file.endswith('.mp4'): 
        ...
        vid_frm_nums, fps_out = utils.vid_2_frames(videos_dir + video_name, frames_dir)  
        ...
        fps = 30 # we select this frame rate just to work with so our window (Mega-group) would be 1801 frames.
        ...
        for i in range(0, len(f_start_shots)):
            shot_ind = i
            fps = fps_out # write videos with fps_out frame per second
